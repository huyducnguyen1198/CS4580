...
def BasicInfo(df):
    ...
    fig.update_layout(title_text="Average Min Temperature by Month")
    fig.show()
    ## Fourth Dot
    printSep("4")
...
def firstCorr(df):
    ...
    print(f"\n{'Spearson':^20}\n{corrS}")
    scale = 10
    df['ScaledMinTemp'] = df['MinTemp'] * 10

    fig = px.line(df, df['ScaledMinTemp'], [df['ScaledMinTemp'] * corrP.values[1][0] ,df['ScaledMinTemp']*corrS.values[1][0]])
    fig.update_traces(name='Pearson',selector=dict(name='wide_variable_0'))
    fig.update_traces(name='Spearman',selector=dict(name='wide_variable_1'))

    fig.add_scatter(x=df['ScaledMinTemp'], y=df['Rainfall'], mode='markers', name='MinTemp vs Rainfall')
    fig.update_xaxes(title_text="MinTemp")
    fig.update_yaxes(title_text="Rainfall")
    fig.update_layout(title_text="Scatter plot and correlation lines(scale MinTemp by 10)")
    fig.show()
...
def secCorr(df):
    ...
    xx = piv[maxPair[0]]
    y = piv[maxPair[1]]
    fig = px.scatter(x=xx, y=y, trendline="ols")
    fig.update_xaxes(title_text=maxPair[0])
    fig.update_yaxes(title_text=maxPair[1])
    fig.update_layout(title_text=f'{maxPair[0]} - {maxPair[1]}')
    fig.show()
    ...
    print(
        "Of all the city, Albury has the most correlation with Mildura as it shows that raining in Albury greatly affects the raining in Mildura.\n"
...
def LGpart1(df):
    ...
    print("Logistic Regression without using feature selection")
...
def LGpart2(df):
    #using RFE to select top 5 feature(Recursive Feature Elimination)
    ...
    #use all numerical data
    x = df[nFeatures].fillna(0)


    #fit and get selected feature
    rfe_fitted = rfe.fit(x, y)
    selected = nFeatures[rfe_fitted.support_]
    print(f"Top 3 Feature: {selected[:3]}")
    print(f"Used Feature: {selected}")
...
def LGpart3PCA(df):
    ...
    print("Using PCA to reduce dimension to 18")
...
def LGpart3LDA(df):
    ...
    print("Using LDA to reduce dimension")
...
def main():
    ...
    firstCorr(df)
    secCorr(df)
    printHeader("Logistic Regression part 1")
    LGpart1(df)
    printHeader("Logistic Regression part 2")
    LGpart2(df)
    printHeader("Logistic Regression part 3 PCA")
    LGpart3PCA(df)
    printHeader("Logistic Regression part 3 LDA")
    LGpart3LDA(df)
    printHeader("XGBoost for highest accuracy possible")
    XGBoost3(df)
